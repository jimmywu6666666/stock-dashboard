...
class StockAnalysisPipeline:
    ...
    def __init__(
        ...
        custom_news_items: Optional[List[Dict[str, Any]]] = None,
        ...
        self.custom_news_items = self._normalize_custom_news_items(custom_news_items)
        self.client_user_id = str(client_user_id).strip() if client_user_id is not None else None
    ...
    def analyze_stock(
        ...
        try:
            ...
            custom_news_items = getattr(self, "custom_news_items", None) or []
            if custom_news_items:
                news_context = self._format_custom_news_context(custom_news_items, stock_name)
                news_result_count = len(custom_news_items)
                logger.info(
                    "%s(%s) 使用调用方提供的新闻/公告上下文: %d 条",
                    stock_name,
                    code,
                    news_result_count,
                )
            ...
            if custom_news_items:
                enhanced_context["custom_news_items"] = custom_news_items
                enhanced_context["custom_news_source"] = "caller_provided"
    ...
    def _build_query_context(self, query_id: Optional[str] = None) -> Dict[str, str]:
        ...
        if self.client_user_id:
            context["client_user_id"] = self.client_user_id
    ...
    def _normalize_custom_news_items(
        self,
        items: Optional[List[Dict[str, Any]]],
    ) -> List[Dict[str, str]]:
        """Normalize caller-provided news/announcement items for prompt context."""
        if not items:
            return []
        ...
        for item in items:
            ...
            normalized.append({
                "title": title[:240],
                "summary": str(item.get("summary") or item.get("content") or "").strip()[:1000],
                "source": str(item.get("source") or "外部资讯").strip()[:80],
                "published_at": str(
                ...
                "type": str(item.get("type") or "news").strip().lower()[:40],
            })
        return normalized[:20]

    def _format_custom_news_context(
        self,
        items: List[Dict[str, str]],
        stock_name: str,
    ) -> str:
        """Render caller-provided Eastmoney news into the existing news_context slot."""
        if not items:
            return ""

        lines = [
            f"【调用方提供的东方财富资讯与公告：{stock_name}】",
            "说明：以下资讯由外部看板传入，已用于替代 DSA 内部第三方新闻搜索；公告类信息可靠性高于普通新闻。",
        ]
        for index, item in enumerate(items, 1):
            item_type = "公告" if item.get("type") == "announcement" else "新闻"
            lines.append(
                f"{index}. [{item_type}] {item.get('title', '')}\n"
                f"   来源：{item.get('source', '')}；时间：{item.get('published_at', '')}\n"
                f"   摘要：{item.get('summary') or item.get('title', '')}\n"
                f"   链接：{item.get('url', '')}"
            )
        return "\n".join(lines)
