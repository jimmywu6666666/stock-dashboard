...
class HistoryService:
    ...
    def _resolve_record(self, record_id: str, client_user_id: Optional[str] = None):
        """
        ...
        try:
            ...
            record = self.db.get_analysis_history_by_id(int_id, client_user_id=client_user_id)
            if record:
                return record
        except (ValueError, TypeError):
            pass
        # Fall back to query_id lookup
        return self.db.get_latest_analysis_by_query_id(record_id, client_user_id=client_user_id)

    def resolve_and_get_detail(self, record_id: str, client_user_id: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """
        ...
        """
        try:
            record = self._resolve_record(record_id, client_user_id=client_user_id)
            if not record:
                return None
    ...
    def delete_history_records(self, record_ids: List[int], client_user_id: Optional[str] = None) -> int:
        """
        ...
        """
        return self.db.delete_analysis_history_records(record_ids, client_user_id=client_user_id)
