...
class CustomNewsItem(BaseModel):
    """调用方提供的新闻/公告条目，用于替代或补充 DSA 内部搜索。"""

    title: str = Field(..., description="新闻或公告标题")
    summary: str = Field("", description="摘要或正文片段")
    source: str = Field("", description="来源，例如 东方财富资讯 / 东方财富公告")
    ...
    type: str = Field("news", description="类型：news / announcement")
...
class AnalyzeRequest(BaseModel):
    ...
    custom_news_items: Optional[List[CustomNewsItem]] = Field(
        None,
        validation_alias=AliasChoices("custom_news_items", "customNewsItems"),
        description="调用方提供的新闻/公告上下文；传入后优先用于 AI 分析，未传时使用 DSA 原搜索逻辑",
    )
    client_user_id: Optional[str] = Field(
        None,
        validation_alias=AliasChoices("client_user_id", "clientUserId"),
        description="调用方用户归属标识，用于隔离嵌入式看板的分析历史",
    )
