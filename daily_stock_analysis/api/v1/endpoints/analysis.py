...
def _handle_async_analysis_batch(
    ...
    custom_news_items = getattr(request, "custom_news_items", None)
    client_user_id = getattr(request, "client_user_id", None)
    ...
    if custom_news_items is not None:
        submit_kwargs["custom_news_items"] = [
            item.model_dump() if hasattr(item, "model_dump") else dict(item)
            for item in custom_news_items
        ]
...
def _handle_sync_analysis(
    ...
    try:
        ...
        result = service.analyze_stock(
            ...
            custom_news_items=[
                item.model_dump() if hasattr(item, "model_dump") else dict(item)
                for item in (getattr(request, "custom_news_items", None) or [])
            ] or None,
            client_user_id=getattr(request, "client_user_id", None),
        )
...
def get_analysis_status(
    task_id: str,
    client_user_id: Optional[str] = Query(None, description="调用方用户归属标识"),
) -> TaskStatus:
    """
    查询分析任务状态
    ...
    if task:
        if client_user_id is not None and getattr(task, "client_user_id", None) != client_user_id:
            raise api_error(404, "not_found", f"任务 {task_id} 不存在或已过期")
    ...
    try:
        ...
        records = db.get_analysis_history(query_id=task_id, limit=1, client_user_id=client_user_id)
