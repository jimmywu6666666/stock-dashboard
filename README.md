...
- “国家队”页基于公开十大股东/十大流通股东快照识别中央汇金、证金、社保、养老金等机构，展示最新持仓、加减仓、估算成本和当前盈亏。主表加减仓比例按同一股票下可比机构合计持股的净变化计算。数据优先使用 Tushare；系统默认每天 20:30 自动刷新，失败后按配置重试；成本优先用首次可见报告期前 90 日 VWAP * 折扣估算，缺少该窗口时用后续首个交易窗口兜底，只适合作为趋势和状态参考。
